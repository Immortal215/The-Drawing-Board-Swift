                                    ...
                                    HStack {
                                        ...
                                        Picker("\(subject)", selection: $subject) {
                                            ForEach(Array(Set(subjects)), id: \.self) { chosenSubject in
                                                ...
                                                if chosenSubject != "Enter new value" && chosenSubject != " " {
                                                    if count > 1 {
                                                        Text ("\(chosenSubject) (\(count))").tag(chosenSubject)
                                                    } else {
                                                        Text(chosenSubject).tag(chosenSubject)
                                                        ...
                                                        HStack {
                                                            ...
                                                            Picker("\(subjects[index])", selection: $subjects[index]) {
                                                                ForEach(Array(Set(subjects)), id: \.self) { chosenSubject in
                                                                    let count = subjects.filter { $0 == chosenSubject }.count
                                                                    if count > 1 {
                                                                        Text ("\(chosenSubject == " " ? "Empty Subject!" : chosenSubject) (\(count))").tag(chosenSubject)
                                                                    } else {
                                                                        Text(chosenSubject).tag(chosenSubject)
